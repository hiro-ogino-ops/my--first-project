...
売れるかどうかは、記事を書く前ではなく書いた後にしか分かりません。読者がまだいない段階で
値段をつけると、売れなかった理由が「値段」なのか「中身」なのか「到達数」なのか切り分けられません。

そこで最初は全文無料で出し、**反応だけを見る期間**を置くことを勧めます。

```yaml
# config/company.yaml
product:
  free_ratio: 1.0   # 有料ラインを作らず全文無料で出す
```

この設定のとき `note.md` に有料ラインの目印は入らず、`meta.json` の `price` は 0 になります。

見るのは売上ではなく次の2つです。

- **最後まで読まれているか**（note のダッシュボードの読了率）
- **保存・スキが伸びるか** — 「あとで使う」と思われた記事は、有料でも買われます

3本出して読了率が低いなら、値段の問題ではありません。テーマか書き方を変えてください。
手応えがあった時点で `free_ratio` を `0.30` 前後に戻し、有料記事に切り替えます。
